...
namespace EventHubProcessor
{
    public class SimulatorTriggerFunction
    {
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "get", "post")] HttpRequestData req)
        {
            ...
            if (!int.TryParse(req.Url.Query.Split('&').FirstOrDefault(q => q.StartsWith("count="))?.Split('=').LastOrDefault(), out int eventCount))
            {
                eventCount = 1000;
            }
            ...
            try
            {
                await using (var producerClient = new EventHubProducerClient(_settings.EventHubConnectionString, _settings.EventHubName))
                {
                    ...
                    _logger.LogInformation($"A total of {totalSentCount} events have been published to {_settings.EventHubName}.");
                }

                var okResponse = req.CreateResponse(HttpStatusCode.OK);
                await okResponse.WriteStringAsync($"Simulation finished. {eventCount} events sent to {_settings.EventHubName}.");
                return okResponse;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error publishing events to {EventHubName}: {ExMessage}", _settings.EventHubName, ex.Message);
                var errorResponse = req.CreateResponse(HttpStatusCode.InternalServerError);
